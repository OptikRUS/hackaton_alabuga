...
class SpaceSeedData:
    ...
    MIN_TASKS_FOR_MISSION = 2
    MIN_BRANCHES_FOR_SECOND_MISSION = 2
    MIN_ARTIFACTS_FOR_SECOND_MISSION = 2
    MIN_SKILLS_FOR_MISSION = 8
    MIN_SKILLS_FOR_THIRD_MISSION = 12
    MIN_COMPETENCIES_FOR_THIRD_MISSION = 3
    MIN_COMPETENCIES_FOR_FOURTH_MISSION = 16

    # Error messages
    SESSION_NOT_INITIALIZED_MSG = "Session not initialized. Call create_session() first."
    ...
    async def seed_ranks(self) -> None:
        ...
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("🚀 Создание космических рангов...")  # noqa: T201
        ...
        for rank_data in ranks_data:
            ...
            existing_rank = await self.session.execute(
                text("SELECT id FROM ranks_rank WHERE name = :name"), {"name": rank_data["name"]}
            )
            if existing_rank.fetchone() is None:
                rank = RankModel(name=rank_data["name"], required_xp=rank_data["required_xp"])
                self.session.add(rank)
                created_count += 1

        await self.session.commit()
        print(f"✅ Создано {created_count} новых рангов")  # noqa: T201

    async def seed_competencies(self) -> None:
        """Создает космические компетенции"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("🌌 Создание космических компетенций...")  # noqa: T201
        ...
        for comp_data in competencies_data:
            ...
            if existing_comp.fetchone() is None:
                competency = CompetencyModel(
                    name=comp_data["name"], max_level=comp_data["max_level"]
                )
                self.session.add(competency)
                created_count += 1

        await self.session.commit()
        print(f"✅ Создано {created_count} новых компетенций")  # noqa: T201

    async def seed_skills(self) -> None:
        """Создает космические навыки"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("⭐ Создание космических навыков...")  # noqa: T201
        ...
        for skill_data in skills_data:
            ...
            existing_skill = await self.session.execute(
                text("SELECT id FROM skills_skill WHERE name = :name"), {"name": skill_data["name"]}
            )
            if existing_skill.fetchone() is None:
                skill = SkillModel(name=skill_data["name"], max_level=skill_data["max_level"])
                self.session.add(skill)
                created_count += 1

        await self.session.commit()
        print(f"✅ Создано {created_count} новых навыков")  # noqa: T201

    async def seed_artifacts(self) -> None:
        """Создает космические артефакты"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("💎 Создание космических артефактов...")  # noqa: T201

        artifacts_data = [
            # Обычные артефакты
            {
                "title": "Кристалл энергии",
                "description": "Мощный источник энергии для корабля",
                "rarity": "common",
                "image_url": "/images/crystal_energy.jpg",
            },
            {
                "title": "Антигравитационный генератор",
                "description": "Позволяет кораблю летать без топлива",
                "rarity": "common",
                "image_url": "/images/antigrav_generator.jpg",
            },
            {
                "title": "Голографический проектор",
                "description": "Создает реалистичные голограммы",
                "rarity": "common",
                "image_url": "/images/holo_projector.jpg",
            },
            # Редкие артефакты
            {
                "title": "Квантовый компьютер",
                "description": "Сверхбыстрый компьютер для сложных вычислений",
                "rarity": "rare",
                "image_url": "/images/quantum_computer.jpg",
            },
            {
                "title": "Телепортационный модуль",
                "description": "Позволяет мгновенно перемещаться на короткие расстояния",
                "rarity": "rare",
                "image_url": "/images/teleport_module.jpg",
            },
            {
                "title": "Временной манипулятор",
                "description": "Может замедлять или ускорять время локально",
                "rarity": "rare",
                "image_url": "/images/time_manipulator.jpg",
            },
            # Эпические артефакты
            {
                "title": "Звездный двигатель",
                "description": "Позволяет путешествовать между галактиками",
                "rarity": "epic",
                "image_url": "/images/stellar_engine.jpg",
            },
            {
                "title": "Планетарный щит",
                "description": "Защищает целую планету от атак",
                "rarity": "epic",
                "image_url": "/images/planetary_shield.jpg",
            },
            {
                "title": "Матрица сознания",
                "description": "Искусственный интеллект с собственной личностью",
                "rarity": "epic",
                "image_url": "/images/consciousness_matrix.jpg",
            },
            # Легендарные артефакты
            {
                "title": "Артефакт Создателей",
                "description": "Древний артефакт неизвестной цивилизации",
                "rarity": "legendary",
                "image_url": "/images/creator_artifact.jpg",
            },
            {
                "title": "Сердце Галактики",
                "description": "Источник энергии всей галактики",
                "rarity": "legendary",
                "image_url": "/images/galaxy_heart.jpg",
            },
            {
                "title": "Ключ от Вселенной",
                "description": "Открывает доступ к секретам мироздания",
                "rarity": "legendary",
                "image_url": "/images/universe_key.jpg",
            },
        ]
        ...
        await self.session.commit()
        print(f"✅ Создано {created_count} новых артефактов")  # noqa: T201

    async def seed_mission_branches(self) -> None:
        """Создает космические сезоны/ветки миссий"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("🌍 Создание космических сезонов...")  # noqa: T201
        ...
        await self.session.commit()
        print(f"✅ Создано {created_count} новых сезонов")  # noqa: T201

    async def seed_tasks(self) -> None:
        """Создает космические задачи"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("📋 Создание космических задач...")  # noqa: T201

        tasks_data = [
            # Базовые задачи
            {
                "title": "Проверить системы корабля",
                "description": "Убедиться, что все системы работают корректно",
            },
            {
                "title": "Заправить топливные баки",
                "description": "Заполнить топливные баки до максимума",
            },
            {
                "title": "Провести диагностику двигателей",
                "description": "Проверить состояние двигательных систем",
            },
            {
                "title": "Калибровка навигационных систем",
                "description": "Настроить системы навигации для точного полета",
            },
            # Исследовательские задачи
            {
                "title": "Сканировать астероидное поле",
                "description": "Исследовать состав и структуру астероидов",
            },
            {
                "title": "Собрать образцы с планеты",
                "description": "Добыть образцы грунта и атмосферы",
            },
            {
                "title": "Изучить аномалию",
                "description": "Исследовать неизвестное космическое явление",
            },
            {
                "title": "Картографировать систему",
                "description": "Создать подробную карту звездной системы",
            },
            # Боевые задачи
            {
                "title": "Уничтожить пиратскую базу",
                "description": "Ликвидировать угрозу космических пиратов",
            },
            {
                "title": "Защитить торговый конвой",
                "description": "Обеспечить безопасность торговых кораблей",
            },
            {
                "title": "Перехватить вражеский корабль",
                "description": "Остановить и захватить вражеское судно",
            },
            {"title": "Оборонять станцию", "description": "Защитить космическую станцию от атак"},
            # Дипломатические задачи
            {
                "title": "Установить контакт с инопланетянами",
                "description": "Наладить дипломатические отношения",
            },
            {
                "title": "Провести переговоры о мире",
                "description": "Достичь мирного соглашения между фракциями",
            },
            {"title": "Обменяться технологиями", "description": "Провести обмен научными знаниями"},
            {
                "title": "Организовать торговый договор",
                "description": "Заключить соглашение о торговле",
            },
        ]
        ...
        await self.session.commit()
        print(f"✅ Создано {created_count} новых задач")  # noqa: T201

    async def seed_missions(self) -> None:
        """Создает космические миссии"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("🎯 Создание космических миссий...")  # noqa: T201
        ...
        competencies = await self.session.execute(
            text("SELECT id FROM competencies_competency ORDER BY id")
        )
        ...
        artifacts = await self.session.execute(
            text("SELECT id FROM artifacts_artifact ORDER BY id")
        )
        ...
        missions_data = [
            {
                "title": "Первый полет",
                "description": (
                    "Ваша первая миссия в космосе. Изучите основы пилотирования и навигации."
                ),
                "reward_xp": 50,
                "reward_mana": 25,
                "rank_requirement": 1,
                ...
                "task_ids": (
                    task_ids[: self.MIN_TASKS_FOR_MISSION]
                    if len(task_ids) >= self.MIN_TASKS_FOR_MISSION
                    else []
                ),
            },
            {
                ...
                "branch_id": (
                    branch_ids[1]
                    if len(branch_ids) > self.MIN_BRANCHES_FOR_SECOND_MISSION
                    else branch_ids[0]
                ),
                "competency_rewards": [
                ...
                "artifact_rewards": (
                    [artifact_ids[1]]
                    if len(artifact_ids) > self.MIN_ARTIFACTS_FOR_SECOND_MISSION
                    else []
                ),
                "task_ids": (
                    task_ids[4 : self.MIN_SKILLS_FOR_MISSION]
                    if len(task_ids) >= self.MIN_SKILLS_FOR_MISSION
                    else []
                ),
            },
            {
                ...
                "task_ids": (
                    task_ids[8 : self.MIN_SKILLS_FOR_THIRD_MISSION]
                    if len(task_ids) >= self.MIN_SKILLS_FOR_THIRD_MISSION
                    else []
                ),
            },
            {
                ...
                "branch_id": (
                    branch_ids[3]
                    if len(branch_ids) > self.MIN_COMPETENCIES_FOR_THIRD_MISSION
                    else branch_ids[0]
                ),
                "competency_rewards": [
                ...
                "artifact_rewards": (
                    [artifact_ids[3]]
                    if len(artifact_ids) > self.MIN_COMPETENCIES_FOR_THIRD_MISSION
                    else []
                ),
                "task_ids": (
                    task_ids[12 : self.MIN_COMPETENCIES_FOR_FOURTH_MISSION]
                    if len(task_ids) >= self.MIN_COMPETENCIES_FOR_FOURTH_MISSION
                    else []
                ),
            },
        ]
        ...
        await self.session.commit()
        print(f"✅ Создано {created_count} новых миссий")  # noqa: T201

    async def seed_store_items(self) -> None:
        """Создает товары для космического магазина"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("🛒 Создание товаров для космического магазина...")  # noqa: T201
        ...
        for item_data in store_items_data:
            ...
            if existing_item.fetchone() is None:
                item = StoreItemModel(
                    title=item_data["title"], price=item_data["price"], stock=item_data["stock"]
                )
                self.session.add(item)
                created_count += 1

        await self.session.commit()
        print(f"✅ Создано {created_count} новых товаров для магазина")  # noqa: T201

    async def seed_admin_user(self) -> None:
        """Создает администратора системы"""
        if self.session is None:
            raise RuntimeError(self.SESSION_NOT_INITIALIZED_MSG)
        print("👤 Создание администратора системы...")  # noqa: T201

        # Проверяем, существует ли уже администратор
        existing_admin = await self.session.execute(
            text("SELECT login FROM users_user WHERE login = :login"), {"login": "admin"}
        )
        if existing_admin.fetchone() is None:
            admin_user = UserModel(
                login="admin",
                password=UserPasswordService().generate_password_hash(
                    "admin123"
                ),  # password: admin123
            ...
            await self.session.commit()
            print("✅ Создан администратор системы (логин: admin, пароль: admin123)")  # noqa: T201
        else:
            print("ℹ️ Администратор уже существует")  # noqa: T201

    async def run_seed(self) -> None:
        """Запускает полное заполнение базы данных"""
        print("🚀 Начинаем заполнение базы данных космической тематикой...")  # noqa: T201
        ...
        try:
            ...
            print("\n🎉 Заполнение базы данных завершено успешно!")  # noqa: T201
            print("🌌 Космическая вселенная готова к исследованию!")  # noqa: T201

        except Exception as e:
            print(f"❌ Ошибка при заполнении базы данных: {e}")  # noqa: T201
