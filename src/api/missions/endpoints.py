...
@router.post(
    path="/missions/{mission_id}/competitions/{competition_id}", status_code=status.HTTP_200_OK
)
...
async def remove_competency_reward_from_mission(
    ...
    mission = await use_case.execute(mission_id=mission_id, competition_id=competition_id)
    return MissionResponse.from_schema(mission=mission)
