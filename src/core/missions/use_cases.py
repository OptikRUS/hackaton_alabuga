...
class AddCompetencyRewardToMissionUseCase(UseCase):
    ...
    async def execute(self, mission_id: int, competition_id: int, level_increase: int) -> Mission:
        await self.storage.add_competency_reward_to_mission(
            mission_id=mission_id, competition_id=competition_id, level_increase=level_increase
        )
        return await self.storage.get_mission_by_id(mission_id=mission_id)
...
class RemoveCompetencyRewardFromMissionUseCase(UseCase):
    ...
    async def execute(self, mission_id: int, competition_id: int) -> Mission:
        await self.storage.remove_competency_reward_from_mission(
            mission_id=mission_id, competition_id=competition_id
        )
        return await self.storage.get_mission_by_id(mission_id=mission_id)
