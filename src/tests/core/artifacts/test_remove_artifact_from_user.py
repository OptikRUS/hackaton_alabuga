...
from src.core.artifacts.use_cases import RemoveArtifactFromUserUseCase
from src.core.users.exceptions import UserNotFoundError
from src.tests.fixtures import FactoryFixture
from src.tests.mocks.storage_stub import StorageMock


class TestRemoveArtifactFromUserUseCase(FactoryFixture):
    @pytest.fixture(autouse=True)
    async def setup(self) -> None:
        self.storage = StorageMock()
        self.use_case = RemoveArtifactFromUserUseCase(
            storage=self.storage, user_storage=self.storage
        )
