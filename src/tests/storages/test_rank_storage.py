...
class TestRankStorage(FactoryFixture, StorageFixture):
    ...
    async def test_get_rank_by_id(self) -> None:
        ...
        assert rank.required_competitions == []
    ...
    async def test_add_remove_required_competition(self) -> None:
        # Create competition to require
        await self.storage.insert_competition(self.factory.competition(name="ML", max_level=5))
        comp = await self.storage.get_competition_by_name(name="ML")

        await self.storage.add_required_competition_to_rank(
            rank_id=self.created.id, competition_id=comp.id, min_level=3
        )
        got = await self.storage.get_rank_by_id(rank_id=self.created.id)
        assert any(
            req.competition.id == comp.id and req.min_level == 3
            for req in (got.required_competitions or [])
        )

        await self.storage.remove_required_competition_from_rank(
            rank_id=self.created.id, competition_id=comp.id
        )
        got2 = await self.storage.get_rank_by_id(rank_id=self.created.id)
        assert all(req.competition.id != comp.id for req in (got2.required_competitions or []))

    async def test_add_required_competition_level_too_high(self) -> None:
        await self.storage.insert_competition(self.factory.competition(name="Web", max_level=2))
        comp = await self.storage.get_competition_by_name(name="Web")
        with pytest.raises(RankCompetitionMinLevelTooHighError):
            await self.storage.add_required_competition_to_rank(
                rank_id=self.created.id, competition_id=comp.id, min_level=99
            )
