...
class TestCreateCompetencyAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_not_auth(self) -> None:
        response = self.api.create_competency(name="TEST", max_level=100)

        assert response.status_code == codes.FORBIDDEN
        assert response.json() == {"detail": "Not authenticated"}

    def test_candidate_forbidden(self) -> None:
        response = self.candidate_api.create_competency(name="TEST", max_level=100)
...
class TestGetCompetenciesAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_get_competencies(self) -> None:
        ...
        response = self.hr_api.get_competencies()
    ...
    def test_get_competencies_empty(self) -> None:
        self.use_case.execute.return_value = self.factory.competencies(values=[])

        response = self.candidate_api.get_competencies()
...
class TestGetCompetencyDetailAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_get_competency_by_id(self) -> None:
        self.use_case.execute.return_value = self.factory.competency(
            competency_id=1,
            name="ML",
            max_level=100,
        ...
        response = self.hr_api.get_competency(competency_id=1)
    ...
    def test_get_competency_not_found(self) -> None:
        self.use_case.execute.side_effect = CompetencyNotFoundError

        response = self.candidate_api.get_competency(competency_id=999)
...
class TestUpdateCompetencyAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_not_auth(self) -> None:
        response = self.api.update_competency(competency_id=1, name="TEST", max_level=150)

        assert response.status_code == codes.FORBIDDEN
        assert response.json() == {"detail": "Not authenticated"}

    def test_candidate_forbidden(self) -> None:
        response = self.candidate_api.update_competency(competency_id=1, name="TEST", max_level=150)

        assert response.status_code == codes.FORBIDDEN
        assert response.json() == {"detail": PermissionDeniedError.detail}

    def test_update_competency(self) -> None:
        self.use_case.execute.return_value = self.factory.competency(
            competency_id=1,
            name="ML Advanced",
            max_level=150,
    ...
    def test_update_competency_name_already_exists(self) -> None:
        self.use_case.execute.side_effect = CompetencyNameAlreadyExistError
    ...
    def test_update_competency_not_found(self) -> None:
        self.use_case.execute.side_effect = CompetencyNotFoundError
...
class TestDeleteCompetencyAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_candidate_forbidden(self) -> None:
        response = self.candidate_api.delete_competency(competency_id=1)

        assert response.status_code == codes.FORBIDDEN
        assert response.json() == {"detail": PermissionDeniedError.detail}

    def test_delete_competency(self) -> None:
    ...
    def test_delete_competency_not_found(self) -> None:
        self.use_case.execute.side_effect = CompetencyNotFoundError
