...
class TestCreateRankAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    def test_create_rank(self) -> None:
        ...
        response = self.api.client.post("/ranks", json={"name": "Bronze", "required_xp": 100})
...
class TestGetRanksAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    async def test_get_ranks(self) -> None:
        ...
        response = self.api.client.get("/ranks")
...
class TestRankDetailAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    async def test_get_rank(self) -> None:
        ...
        response = self.api.client.get("/ranks/1")
    ...
    async def test_get_rank_not_found(self) -> None:
        ...
        response = self.api.client.get("/ranks/999")
...
class TestUpdateRankAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    async def test_update_rank(self) -> None:
        ...
        response = self.api.client.put("/ranks/1", json={"name": "Silver", "required_xp": 200})
        assert response.status_code == codes.OK
        self.use_case.execute.assert_awaited_once_with(
    ...
    async def test_update_rank_not_found(self) -> None:
        self.use_case.execute.side_effect = RankNotFoundError
        response = self.api.client.put("/ranks/999", json={"name": "Silver", "required_xp": 200})
        assert response.status_code == codes.NOT_FOUND
...
class TestRankRelationsAPI(APIFixture, FactoryFixture, ContainerFixture):
    ...
    async def test_add_required_mission(self) -> None:
        ...
        response = self.api.client.post("/ranks/1/missions/10")
        assert response.status_code == codes.OK
    ...
    async def test_remove_required_mission(self) -> None:
        ...
        response = self.api.client.delete("/ranks/1/missions/10")
        assert response.status_code == codes.OK
        self.remove_mission_uc.execute.assert_awaited_once_with(rank_id=1, mission_id=10)

    async def test_add_required_competition(self) -> None:
        self.add_comp_uc.execute.return_value = self.factory.rank(
            rank_id=1, name="Bronze", required_xp=100
        )
        response = self.api.client.post("/ranks/1/competitions/5?min_level=3")
        assert response.status_code == codes.OK
        self.add_comp_uc.execute.assert_awaited_once_with(rank_id=1, competition_id=5, min_level=3)

    async def test_add_required_competition_level_too_high(self) -> None:
        self.add_comp_uc.execute.side_effect = RankCompetitionMinLevelTooHighError
        response = self.api.client.post("/ranks/1/competitions/5?min_level=9999")
        assert response.status_code == codes.BAD_REQUEST

    async def test_remove_required_competition(self) -> None:
        self.remove_comp_uc.execute.return_value = self.factory.rank(
            rank_id=1, name="Bronze", required_xp=100
        )
        response = self.api.client.delete("/ranks/1/competitions/5")
        assert response.status_code == codes.OK
        self.remove_comp_uc.execute.assert_awaited_once_with(rank_id=1, competition_id=5)
